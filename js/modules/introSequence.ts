...
export class IntroSequence {
    ...
    constructor(scene: THREE.Scene, camera: THREE.Camera, spaceship: SpaceshipLike | null, audioManager: AudioManagerLike | null) {
        ...
        if (DEBUG_MODE.enabled) console.log("Intro sequence initialized");
    ...
    startSequence(onComplete?: () => void): void {
        ...
        if (DEBUG_MODE.enabled) console.log("Starting intro sequence...");
        this.isPlaying = true;
        this.sequenceTime = 0;
    ...
    skipSequence(): void {
        if (DEBUG_MODE.enabled) console.log("Skipping intro sequence");
        ...
        if (this.spaceship && this.spaceship.mesh) {
            ...
            if (this.spaceship.hull !== undefined && this.spaceship.hull <= 0) {
                if (DEBUG_MODE.enabled) console.log("Fixing spaceship hull from", this.spaceship.hull, "to 100");
                this.spaceship.hull = 100;
            }
            if (this.spaceship.shield !== undefined && this.spaceship.shield <= 0) {
                if (DEBUG_MODE.enabled) console.log("Fixing spaceship shield from", this.spaceship.shield, "to 50");
                this.spaceship.shield = 50;
            }
            if (this.spaceship.fuel !== undefined && this.spaceship.fuel <= 0) {
                if (DEBUG_MODE.enabled) console.log("Fixing spaceship fuel from", this.spaceship.fuel, "to 100");
        ...
        if (gameWindow.gameInstance && gameWindow.gameInstance.spaceship) {
            ...
            if (DEBUG_MODE.enabled) console.log("Ship set to undocked state after skip");
            ...
            if (gameWindow.gameInstance.spaceship.hull !== undefined && gameWindow.gameInstance.spaceship.hull <= 0) {
                if (DEBUG_MODE.enabled) console.log("Fixing global spaceship hull to 100");
            ...
            if (gameWindow.gameInstance.spaceship.fuel !== undefined && gameWindow.gameInstance.spaceship.fuel <= 0) {
                if (DEBUG_MODE.enabled) console.log("Fixing global spaceship fuel to 100");
    ...
    completeSequence(): void {
        if (DEBUG_MODE.enabled) console.log("Intro sequence complete");
        this.isPlaying = false;
        ...
        if (this.spaceship && this.spaceship.mesh) {
            ...
            if (this.spaceship.isDocked) {
                if (DEBUG_MODE.enabled) console.log("Forcing ship to undocked state after intro sequence");
                this.spaceship.isDocked = false;
            }
            
            // Log the final player position for debugging
            if (DEBUG_MODE.enabled) console.log("Player final position:", 
        ...
        if (this.onComplete && typeof this.onComplete === 'function') {
            ...
            setTimeout(() => {
                if (DEBUG_MODE.enabled) console.log("Executing intro sequence completion callback");
                this.onComplete?.();
            }, 100);
        }
    }

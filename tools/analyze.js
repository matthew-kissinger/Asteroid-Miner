...
function buildRepoMap() {
  ...
  for (const top of fs.readdirSync(ROOT, { withFileTypes: true })) {
    ...
    // collect children listing up to depth 2 for map
    const level1 = fs.readdirSync(topPath, { withFileTypes: true })
...
function parseImportBindings(content) {
  // Returns array of { source, defaultImport, named: [{imported, local}] }
  const results = [];
  ...
  while ((m = re.exec(content)) !== null) {
    ...
    if (clause.startsWith('{')) {
      ...
      // default import, possibly along with named
...
function main() {
  ...
  // dep graph and public surface and events
  const depEdges = [];
  const exportsMap = new Map(); // file -> Set(exports)
  const importUse = new Map(); // file -> array of { source,file, default,named[] }
  const events = []; // { type, topic, file, line, snippet }
  const funcSizes = [];
  ...
  for (const abs of files) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (pub) {
        events.push({ type: 'publish', topic: pub[2], file: relPath, line: i + 1, snippet: line.trim().slice(0, 200) });
      }
      const sub = line.match(/\.(?:subscribe)\(\s*(["'])(.*?)\1/);
      if (sub) {
        events.push({ type: 'subscribe', topic: sub[2], file: relPath, line: i + 1, snippet: line.trim().slice(0, 200) });
      }
    }
    ...
    for (const b of blocks) {
      funcSizes.push({ path: relPath, function: b.name, loc: b.loc });
    }
  }
  ...
  // Consumers per export
  const exportConsumers = new Map(); // key: file::symbol -> Set(consumer file)
